...
there. When the week resolves, your token gambles --- a single coin flip decides the
outcome:

\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Heads (50\%)} --- the token grants \textbf{+3 bonus points}, added straight to your weekly score.
    \item \textbf{Tails (50\%)} --- the token grants \textbf{+0 points}. Nothing happens.
\end{itemize}

The channel you place the token on doesn't affect the outcome --- it's purely a
quantum coin flip. The expected payoff is 1.5 points, so the token is usually
worth using, but you might just get unlucky.

This mirrors a real two-state quantum system: until you measure it, the particle
is in \emph{both} states (winning and losing) at once. Measurement collapses it
to one outcome.
...
\begin{center}
    \small\color{qcmuted}
    Waterloo Quantum Club --- play at \href{https://localhost:8080/game.html}{game.html}\\
